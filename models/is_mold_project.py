...
class is_mold_project(models.Model):
    ...
    name           = fields.Char("Nom du projet",size=40,required=True, index=True, tracking=True)
    client_id      = fields.Many2one('res.partner', 'Client', tracking=True)
    chef_projet_id = fields.Many2one('res.users', 'Chef de projet',required=True, tracking=True)
    choix_modele   = fields.Selection([
            ('1', u'1 - Moule par défaut hors automobile'),
            ('2', u'2 - Moule par défaut automobile'),
        ], "Choix du modèle",required=True, tracking=True)
