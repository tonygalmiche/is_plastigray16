...
class IsFactureProformaExportTunisieLigne(models.Model):
    ...
    def _get_default_categorie(self):
        """Retourne la dernière catégorie utilisée"""
        categorie = self.env['is.mem.var'].get(self._uid, 'is.facture.proforma.export.tunisie.categorie')
        return categorie
    ...
    def onchange_categorie(self):
        if self.categorie:
            self.env['is.mem.var'].set(self._uid, 'is.facture.proforma.export.tunisie.categorie', self.categorie)
    ...
    @api.onchange('num_colis')
    def onchange_num_colis(self):
        num_colis_id = False
        if self.num_colis and self.proforma_id:
            # Si la proforma n'est pas encore sauvée (NewId), chercher dans les colisages en mémoire
            if isinstance(self.proforma_id.id, models.NewId):
                # Rechercher dans les colisages en mémoire de la proforma
                for colisage in self.proforma_id.colisage_ids:
                    if colisage.num_colis == self.num_colis:
                        num_colis_id = colisage
                        break
            else:
                # Proforma sauvée, rechercher en base de données
                colisage = self.env['is.facture.proforma.export.tunisie.colisage'].search([
                    ('proforma_id', '=', self.proforma_id.id),
                    ('num_colis', '=', self.num_colis)
                ], limit=1)
                if colisage:
                    num_colis_id = colisage.id
        self.num_colis_id = num_colis_id

    def copier_ligne_action(self):
        """Copie la ligne en cours et l'insère juste après"""
        self.ensure_one()
        
        # Récupérer toutes les lignes triées par num (numéro de ligne)
        toutes_lignes = self.proforma_id.ligne_ids.sorted('num')
        
        # Trouver le numéro de la ligne courante
        num_courant = self.num
        
        # Décaler les numéros des lignes suivantes pour faire de la place
        lignes_a_decaler = toutes_lignes.filtered(lambda l: l.num > num_courant)
        for ligne in lignes_a_decaler:
            ligne.write({'num': ligne.num + 1})
        
        # Copier la ligne avec le bon numéro (juste après la ligne courante)
        nouvelle_ligne = self.copy({'num': num_courant + 1})