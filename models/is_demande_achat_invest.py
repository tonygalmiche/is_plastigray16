...
class is_demande_achat_invest(models.Model):
    ...
    _description="Demande d'achat d'investissement"
    ...
    name                 = fields.Char("N°DA-I", tracking=True, readonly=True)
    createur_id          = fields.Many2one('res.users', 'Demandeur', tracking=True, copy=False, required=True, default=lambda self: self.env.uid)
    chef_service_id      = fields.Many2one('res.users', 'Chef de service', tracking=True, required=True)
    direction_id         = fields.Many2one('res.users', 'Direction', tracking=True, readonly=True, default=lambda self: self._dirigeant_id())
    ...
    fournisseur_id       = fields.Many2one('res.partner', 'Fournisseur', tracking=True, domain=[('is_company','=',True),('is_segment_achat.name','=',u"Fournisseurs d'Investissements")])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['name'] = self.env['ir.sequence'].next_by_code('is.demande.achat.invest')
...
class is_demande_achat_invest_line(models.Model):
    ...
    _description="Lignes demande d'achat d'investissement"
