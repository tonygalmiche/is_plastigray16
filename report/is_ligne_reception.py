        ...
        cr.execute("""
            CREATE OR REPLACE view is_ligne_reception AS (
                select  sm.id,
                        ...
                        round(sm.product_uom_qty-coalesce((select sum(quantity) from account_move_line ail where parent_state='posted' and ail.is_move_id=sm.id ),0),4)*pol.price_unit as montant_reste,
                        sm.state              as state,
