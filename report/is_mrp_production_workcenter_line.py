...
class is_mrp_production_workcenter_line(models.Model):
    ...
    _description="Postes de charges"
    ...
    mpwl_id            = fields.Many2one('mrp.workorder', 'Ordre de travail')
        ...
        cr.execute("""
            CREATE OR REPLACE view is_mrp_production_workcenter_line AS (
                select
                    ...
                    mpwl.duration_expected/60 as hour,
                    -- mpwl.cycle            as cycle,
                    mpwl.is_ordre         as is_ordre,
                    mpwl.is_date_tri      as is_date_tri,
                    mpwl.is_date_planning as is_date_planning
                from mrp_workorder mpwl inner join mrp_production mp on mpwl.production_id=mp.id
