...
class report_stock_lot_bloque(osv.osv):
    ...
    def init(self, cr):
        ...
        cr.execute("""
                CREATE OR REPLACE FUNCTION is_qt_par_uc2(product_template_id integer, OUT uc int) RETURNS int AS $$
                BEGIN
                    uc:=cast((select qty from product_packaging where product_tmpl_id=product_template_id order by id limit 1) as int);
                    IF uc=0 THEN
                        uc:=1;
                    END IF;
                    IF   uc IS NULL THEN
                ...
                GROUP BY quant.lot_id, quant.product_id, pp.product_tmpl_id, quant.location_id, im.name
